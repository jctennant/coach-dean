...
- Current weekly mileage (recent average): ${user.current_weekly_miles} mi/week
...
${buildPlanGroundTruthBlock(ground_truth)}
`.trim();
...
SCORE RUBRIC:
- 10: Plan is well-calibrated — right volume, right session types, safe progression, long run appropriate
- 7-9: Mostly sound with minor issues (slightly aggressive week 1, one session type missing)
- 4-6: One significant structural problem (long run too long, peak volume inappropriate, wrong quality work)
...
Return ONLY valid JSON — no preamble, no explanation outside the JSON:
{
  ...
  "week1_miles_stated": "the week 1 total mileage as stated, e.g. '18 mi' — or null if not stated",
  "peak_volume_appropriate": true | false | null,
  "peak_miles_stated": "peak week mileage as stated, e.g. '45 mi' — or null if not stated",
  ...
  "long_run_miles_stated": "longest run distance mentioned, e.g. '6 mi' — or null if not stated",
  "quality_sessions_appropriate": true | false | null,
  "quality_sessions_stated": ["list the quality session types mentioned, e.g. 'interval 400m reps', 'tempo 3mi'"],
  "progression_safe": true | false | null,
...
function buildPlanGroundTruthBlock(gt) {
  if (!gt) return "No specific ground truth expectations.";
  const lines = [];
  if (gt.max_week1_miles != null) lines.push(`- Week 1 MUST NOT exceed: ${gt.max_week1_miles} mi`);
  if (gt.min_week1_miles != null) lines.push(`- Week 1 should be at least: ${gt.min_week1_miles} mi`);
  if (gt.max_peak_weekly_miles != null) lines.push(`- Peak week MUST NOT exceed: ${gt.max_peak_weekly_miles} mi`);
  if (gt.min_peak_weekly_miles != null) lines.push(`- Peak week should reach at least: ${gt.min_peak_weekly_miles} mi`);
  if (gt.max_long_run_miles != null) lines.push(`- Longest run MUST NOT exceed: ${gt.max_long_run_miles} mi`);
  if (gt.min_long_run_miles != null) lines.push(`- Longest run should reach at least: ${gt.min_long_run_miles} mi`);
