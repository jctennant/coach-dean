...
export function ExerciseList({ exercises, token, initialDone }: Props) {
  ...
  async function markDone(exerciseId: string) {
    if (done.has(exerciseId) || pending.has(exerciseId)) return;
    // Optimistic update
    setDone((prev) => new Set([...prev, exerciseId]));
    setPending((prev) => new Set([...prev, exerciseId]));
    try {
      await fetch("/api/session/complete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, exerciseId }),
      });
    } finally {
      setPending((prev) => {
  ...
  return (
    <div className="px-5 py-6">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className={`text-xs mt-0.5 ${isDone ? "text-green-600" : "text-gray-500"}`}>
                    {ex.specs}
                  </p>
                  <p className={`text-xs mt-1 italic leading-relaxed ${isDone ? "text-green-500" : "text-gray-400"}`}>
                    {ex.cue}
                  </p>
                </div>
              </div>
      ...
      {allDone && (
        <div className="mt-6 rounded-xl bg-green-50 border border-green-200 px-5 py-5 text-center">
          <p className="text-lg font-bold text-green-700">All done</p>
          <p className="text-sm text-green-600 mt-1">Nice work. See you next run.</p>
        </div>
      )}
    </div>
  );
