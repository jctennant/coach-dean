 ...
 */
function computePhaseForPlan(weekNumber: number, totalWeeks: number, hasRace: boolean): string {
  if (!hasRace) {
 ...
 * Also generates a dashboard_token, sets trial_started_at, and texts the plan link.
 * Called after the initial_plan SMS is sent — non-blocking for the user experience.
 */
async function generateAndSaveFullPlan(
  userId: string,
  phoneNumber: string,
  ...
  // Determine total weeks: race distance → cap 4–24; no race → 12
  let totalWeeks = 12;
  if (raceDate) {
    const now = new Date();
  ...
  for (let week = 1; week <= totalWeeks; week++) {
    ...
    // Deload every 4th week except in peak/taper phases
    ...
    // Long run: fraction of weekly mileage, varies by phase
...
No other text.`,
    ...
    // Leave key_workout/notes empty — plan arc still saves
  }
