...
Return ONLY valid JSON: {"goal_time_minutes": number | null, "has_answered": boolean}`,
  ...
  if (goalTimeMinutes === null) {
    ...
    // General fallback: if no specific branch handled it and the user hasn't answered,
    // generate a contextual re-ask rather than silently advancing. Handles race results,
    // off-topic messages, or anything else that didn't yield a clear personal goal time.
    if (!hasAnswered) {
      const reaskName = onboardingData.race_name as string | null;
      const reaskResponse = await anthropic.messages.create({
        model: "claude-haiku-4-5-20251001",
        max_tokens: 100,
        system: `You are Coach Dean, an AI running coach. An athlete was asked what their finish time goal is${reaskName ? ` for the ${reaskName}` : ""} but their reply didn't include a clear personal goal time. In 1-2 short, plain-text sentences, briefly acknowledge what they shared (if anything substantive) and re-ask what time they personally are targeting, or whether they'd rather focus on finishing strong. Conversational, no markdown.`,
        messages: [{ role: "user", content: message }],
      });
      const reaskText = reaskResponse.content[0].type === "text" ? reaskResponse.content[0].text.trim() : "";
      if (reaskText && !/^\s*\{/.test(reaskText)) {
        await sendAndStore(user.id, user.phone_number, reaskText, "awaiting_goal_time");
        return NextResponse.json({ ok: true });
      }
    }
  }
