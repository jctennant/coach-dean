...
export async function POST(request: Request) {
  const { secret, userId, trigger = "initial_plan", dry_run = false, activityId, ...extraFlags } = await request.json();
  ...
  console.log(`[admin/trigger] firing trigger="${trigger}" for userId=${userId}`);

  const res = await fetch(`${appUrl}/api/coach/respond`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ userId, trigger, dry_run, ...(activityId ? { activityId } : {}), ...extraFlags }),
  });
