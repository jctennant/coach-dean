...
| `/api/cron/missed-messages` ⛔ | `*/30 * * * *` (every 30 min) | Re-dispatches inbound user messages 3–90 min old with no assistant reply. Fully-onboarded users → `coach/respond`; users mid-onboarding (`onboarding` / `awaiting_strava` / `awaiting_timezone`) → `onboarding/handle` with `retry: true`, replaying the original message body. |
